...
ship **`_macos` only** in the PR; the `_linux` triples are generated by the
`regenerate-linux-goldens` workflow (manual `workflow_dispatch` on the PR
branch — see *Regenerating the `_linux` fixtures* below). On Linux/CI the
missing fixtures fail the relevant `GoldenMaster.*` tests loudly until the
