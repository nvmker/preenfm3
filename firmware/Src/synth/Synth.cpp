 ...
 * end pointers are computed once as bufferN + 64. The warned loops — the
 * zero-fill, the dispatch cases 0/2/3/5/6/8 (two increments per
 * iteration), and the clip/x256 pass — compare a monotonically advancing
 * pointer against the fixed end pointer; mixAndPan() (cases 1/4/7) advances
 * by index (2 dest increments x BLOCK_SIZE=32 = 64 elements) and raises no
 * warning. Every loop stops at or before endcbN. All pointer arithmetic
 * stays within
