...
- [x] [Review][Patch] Payload read failures still partially mutate `actions`/`stepNotes`, violating the unchanged-state acceptance criterion [`firmware/Src/filesystem/SequenceBank.cpp:179-184/:209-214`] — fixed with 16 KiB/24 KiB transactional staging buffers split across verified D3/D2B linker headroom; live state commits only after all reads succeed
- [x] [Review][Patch] Zero-fill write failure exits only the inner loop, so later sequence slots are written at shifted offsets; the regression currently fails the header write instead of the guarded loop [`firmware/Src/filesystem/SequenceBank.cpp:338-346`, `tests/sequence_bank_test.cpp:290-297`] — fixed by closing/returning on failure and targeting the third `f_write` with an exact short-file assertion
- [x] [Review][Patch] Name-read regression consumes its injected failure on the version header, leaving the 20-byte payload-read validation untested [`tests/sequence_bank_test.cpp:444-454`] — fixed with second-read failed/short injections for both V1 and V2
- [x] [Review][Patch] CI Clang builds reject undeclared `INFINITY`/`NAN` macros in the folded-B regression [`tests/lfo_step_seq_test.cpp:306-307`] — fixed with portable `std::numeric_limits<float>` values
- [x] [Review][Defer] `createSequenceFile` still ignores header/state `f_write` failures, allowing malformed banks [`firmware/Src/filesystem/SequenceBank.cpp:313/:329`] — deferred, pre-existing
...
- **Folded-A atomicity:** the 1024-B state goes through `storageBuffer`; `actions` and `stepNotes` use dedicated 16 KiB/24 KiB staging arrays split across D3/D2B SRAM. An `f_size` pre-check rejects truncation, every read requires `FR_OK` plus exact length, and live state is committed only after all reads succeed.
