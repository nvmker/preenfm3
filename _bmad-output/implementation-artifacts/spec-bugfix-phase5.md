...
- [ ] `PatchBank.cpp` + `patch_bank_test.cpp` — single `(char*)`-subtraction (or `offsetof`) form, drop `#ifndef PFM3_HOST` shim; V1 tests stay green — 5.5
- [ ] `SequenceBank.cpp` + `sequence_bank_test.cpp` — bound the zero-fill loop (break on `FR != FR_OK || byteWritten != toWrite`) — 5.6
